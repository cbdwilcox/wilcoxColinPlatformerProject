...
public class EliteScript : MonoBehaviour
{
    ...
    private enum State { Idle, SpinPrep, Spin, Block }
    private State state;
    ...
    void Update()
    {
        ...
        else if (!Stagger)
        {
            ...
            if (Mathf.Abs(distance) > 4.5 && Mathf.Abs(distance) < 14.5f && Alert && !Stagger && state != State.Block || Mathf.Abs(distance) > 21 && Alert && !HitCool && state != State.Block)
            {
    ...
    void Die()
    {
        Debug.Log("Arcturus Elite Soldier down...");
