...
public class NovaGremlinScript : MonoBehaviour
{
    Color baseColor;
    ...
    private enum State { Idle, Lumber, Claw }
    private State state;
    ...
    void Update()
    {
        ...
        else if (!Stagger)
        {
            ...
            else if (Mathf.Abs(distance) > 4.5 && Mathf.Abs(distance) < 14.5f && Alert && isGrounded || Mathf.Abs(distance) > 21 && Alert && isGrounded)
            {
                ...
                if (state == State.Lumber && FacingRight && !PlayerInvuln && isGrounded)
                {
                    rb2d.velocity = Vector2.zero;
